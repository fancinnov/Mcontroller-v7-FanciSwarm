...
static bool rf_alt_raw_healthy=false;
...
static float pitch_log_deg=0 , roll_log_deg=0 , yaw_log_deg=0;
static float cos_roll=0, cos_pitch=0, cos_yaw=0, sin_roll=0, sin_pitch=0, sin_yaw=0;
static float cos_roll_log=0, cos_pitch_log=0, cos_yaw_log=0, sin_roll_log=0, sin_pitch_log=0, sin_yaw_log=0;
...
EKF_Wind *ekf_wind=new EKF_Wind(_dt, 0.000016, 0.0016, 0.0016);
...
float log_pitch_deg(void){return pitch_log_deg;}
float log_roll_deg(void){return roll_log_deg;}
float log_yaw_deg(void){return yaw_log_deg;}
float log_cos_roll(void){return cos_roll_log;}
float log_sin_roll(void){return sin_roll_log;}
float log_cos_pitch(void){return cos_pitch_log;}
float log_sin_pitch(void){return sin_pitch_log;}
float log_cos_yaw(void){return cos_yaw_log;}
float log_sin_yaw(void){return sin_yaw_log;}
...
float get_dis_wall(void){return rf_dis_wall;}
...
void update_dataflash(void){
	...
	set_fs_rid(param->fs_uid_type.value, sn_num, param->real_name_reg.value, param->drone_activated.value);
}
...
static bool get_optical_location=false;
void opticalflow_update(void){
	...
	if(lc302_data.quality==245){
		...
		if((!get_gnss_state()||!USE_MAG)&&get_perching()!=PERCHING_GROUND){
			if((robot_state==STATE_TAKEOFF||robot_state==STATE_FLYING)&&!odom_safe){
...
//	usb_printf_dir("$%d %d;", lc302_data.flow_x_integral, (int16_t)(constrain_float(flow_gyro_offset.x*flow_gain_y*param->flow_gain.value.y, -0.2, 0.2)*10000));
	//机体坐标系->大地坐标系
	opticalflow_state.rads.x=flow_bf_x*log_cos_yaw()-flow_bf_y*log_sin_yaw();
	opticalflow_state.rads.y=flow_bf_x*log_sin_yaw()+flow_bf_y*log_cos_yaw();
...
static mavlink_set_actuator_control_target_t actuator;
	...
	if (mavlink_parse_char(chan, data, msg_received, status)){
		...
		switch (msg_received->msgid) {
			case MAVLINK_MSG_ID_HEARTBEAT:
				...
				if((msg_received->compid>=MAV_COMP_ID_USER1)&&(msg_received->compid<=MAV_COMP_ID_USER20)){
					heartbeat_req_id=msg_received->compid;
					if(param->uwb_tag_id.value-1+MAV_COMP_ID_USER1==heartbeat_req_id){
						send_mavlink_heartbeat_data(heartbeat_req_id);
						send_mavlink_data(chan, heartbeat_req_id);
					}
				}else{
					heartbeat_req_id=MAV_COMP_ID_AUTOPILOT1;
				}
				break;
...
//							param->rate_pid_integrator.value={0.0f,0.0f,0.0f};
					...
					case MAV_CMD_DO_SET_PARAMETER:
						...
						case 52:
							...
							dataflash->set_param_uint8(param->drone_activated.num,param->drone_activated.value);
							if(param->drone_activated.value==0){//重置激活时间和实名登记
								param->activated_time.value=0;
								dataflash->set_param_uint32(param->activated_time.num,param->activated_time.value);
								param->real_name_reg.value=0;
								dataflash->set_param_uint32(param->real_name_reg.num,param->real_name_reg.value);
								set_fs_rid(param->fs_uid_type.value, sn_num, param->real_name_reg.value, param->drone_activated.value);
							}
							command_long.command=MAV_CMD_DO_SET_PARAMETER;
						...
						case 55:
							...
							dataflash->set_param_uint8(param->fs_uid_type.num, param->fs_uid_type.value);//产品型号
							command_long.command=MAV_CMD_DO_SET_PARAMETER;
			...
			case MAVLINK_MSG_ID_SET_POSITION_TARGET_LOCAL_NED:     // MAV ID: 84
				...
				case MAV_FRAME_MISSION:
					if(offboard_connected){
						...
						if(robot_state==STATE_FLYING||!motors->get_interlock()){//只有飞行中和完全锁定允许下发ego目标
...
//							usb_printf("pos:%f,%f,%f\n",set_goal_point.x,set_goal_point.y,set_goal_point.z);
				...
				default:
					...
					mav_x_target=set_position_target_local_ned.x * 100.0f;//接收的外部目标必须是NEU或者FRU坐标系,单位是m,先转为cm
					...
					mav_yaw_target=set_position_target_local_ned.yaw*RAD_TO_DEG;//目标偏航角正方向为向右转,为NED或FRD坐标系
...
void send_mavlink_heartbeat_data(uint8_t req_id){
	if(req_id!=heartbeat_req_id){
		return;
	}
	mavlink_system.sysid=1;
	mavlink_system.compid=req_id;
...
void send_mavlink_data(mavlink_channel_t chan, uint8_t req_id)
{
	...
	if((time-time_last_heartbeat[(uint8_t)chan])>5000){
	...
	if(uwb->uwb_mode==range){
		for(uint8_t i=1;i<=param->uwb_tag_max.value;i++){
			if(i!=uwb->TAG_ID){
				...
				command_long.param2=(float)i;
				command_long.param3=(float)uwb->get_range_distance(uwb->TAG_ID,i);//cm
				mavlink_msg_command_long_encode(mavlink_system.sysid, mavlink_system.compid, &msg_command_long, &command_long);
				mavlink_send_buffer(chan, &msg_command_long);
			}
		}
...
void send_heartbeat_request(uint8_t req_id){
  ...
  mavlink_msg_heartbeat_encode(1, req_id, &msg_heartbeat, &heartbeat);
  mavlink_send_buffer(MAVLINK_COMM_1, &msg_heartbeat);
}
...
void distribute_mavlink_data(void){
	...
	if (HeartBeatFlags&EVENTBIT_HEARTBEAT_COMM_0){
		send_mavlink_data(MAVLINK_COMM_0, MAV_COMP_ID_AUTOPILOT1);
	}
#endif
#if COMM_1==MAV_COMM
	if (HeartBeatFlags&EVENTBIT_HEARTBEAT_COMM_1){
		send_mavlink_data(MAVLINK_COMM_1, MAV_COMP_ID_AUTOPILOT1);
	}
#endif
#if COMM_2==MAV_COMM
	if (HeartBeatFlags&EVENTBIT_HEARTBEAT_COMM_2){
		send_mavlink_data(MAVLINK_COMM_2, MAV_COMP_ID_AUTOPILOT1);
	}
#endif
#if COMM_3==MAV_COMM
	if (HeartBeatFlags&EVENTBIT_HEARTBEAT_COMM_3){
		send_mavlink_data(MAVLINK_COMM_3, MAV_COMP_ID_AUTOPILOT1);
	}
#endif
#if COMM_4==MAV_COMM
	if (HeartBeatFlags&EVENTBIT_HEARTBEAT_COMM_4){
		send_mavlink_data(MAVLINK_COMM_4, MAV_COMP_ID_AUTOPILOT1);
	}
#endif
#if USE_CH8_SERVO
...
void ahrs_update(void){
	...
	if(ahrs_stage_compass){
		...
		roll_log_deg=roll_log*RAD_TO_DEG;
		pitch_log_deg=pitch_log*RAD_TO_DEG;
		yaw_log_deg=yaw_log*RAD_TO_DEG;
		...
		cos_roll_log=cosf(roll_log);
		sin_roll_log=sinf(roll_log);
		cos_pitch_log=cosf(pitch_log);
		sin_pitch_log=sinf(pitch_log);
		cos_yaw_log=cosf(yaw_log);
		sin_yaw_log=sinf(yaw_log);
		ax_body=0.5*ax_body+0.5*(accel_ef_filt.x*cos_yaw_log + accel_ef_filt.y*sin_yaw_log);
		ay_body=0.5*ay_body+0.5*(-accel_ef_filt.x*sin_yaw_log + accel_ef_filt.y*cos_yaw_log);
		if(mag_correcting){
#if USE_MAG
			compass_calibrate();
#endif
...
//		usb_printf_dir("$%d %d;", (int16_t)(ned_current_vel.x),(int16_t)(ned_current_vel.y));
		if(USE_MAG){
			if(robot_state!=STATE_TAKEOFF&&robot_state!=STATE_FLYING){
				if(robot_state==STATE_LANDED&&PERCHING_MODE){
		...
		//机体坐标系->大地坐标系
		gnss_gyro_offset.x=gyro_filt.x*log_cos_yaw()-gyro_filt.y*log_sin_yaw();
		gnss_gyro_offset.y=gyro_filt.x*log_sin_yaw()+gyro_filt.y*log_cos_yaw();
...
static float wall_pos_z=0.0f,wall_vel_z=0.0f;
...
//	usb_printf_dir("$%d %d %d %d;", uwb->Anchordistance[0], uwb->Anchordistance[1], uwb->Anchordistance[2], uwb->Anchordistance[3]);
	...
	if(ekf_baro->vel_2d<100.0f&&robot_state!=STATE_STOP){
...
//400hz
static float distance_wheel=0, distance_wheel_last=0, distance_wheel_delta=0;
static float wheel_radius=10.0f;//cm
void encoder_position_update(void){
#if USE_ENCODER
	if(get_perching()==PERCHING_GROUND){
		distance_wheel=(get_left_theta()+get_right_theta())*wheel_radius/2;
//		usb_printf("r:%f|%f",get_left_theta(),get_right_theta());
		distance_wheel_delta=distance_wheel-distance_wheel_last;
		ned_current_vel.x=distance_wheel_delta*cos_yaw_log/_dt;
		ned_current_vel.y=distance_wheel_delta*sin_yaw_log/_dt;
		ned_current_pos.x+=distance_wheel_delta*cos_yaw_log;
		ned_current_pos.y+=distance_wheel_delta*sin_yaw_log;
		distance_wheel_last=distance_wheel;
//		usb_printf("pos:%f|%f",ned_current_pos.x,ned_current_pos.y);
		get_gnss_location=true;
		wall_pos_z=get_pos_z();
	}else if(get_perching()==PERCHING_WALL){
		distance_wheel=(get_left_theta()+get_right_theta())*wheel_radius/2;
//		usb_printf("r:%f|%f",get_left_theta(),get_right_theta());
		distance_wheel_delta=distance_wheel-distance_wheel_last;
		wall_vel_z=distance_wheel_delta*cos_roll_log/_dt;
		wall_pos_z+=distance_wheel_delta*cos_roll_log;
		distance_wheel_last=distance_wheel;
		get_gnss_location=true;
	}else{
		wall_pos_z=get_pos_z();
...
static float odom_pos_x=0.0f, odom_pos_y=0.0f, odom_vel_x=0.0f, odom_vel_y=0.0f, odom_acc_gain=0.02f, odom_optical_gain=0.03f;
...
static bool get_odom_location=false;
...
#else
	if(!ahrs->is_initialed()||(!ahrs_healthy)||!USE_FLOW){
		return;
	}
	if((!get_gnss_state())&&get_perching()!=PERCHING_GROUND){
		if(!odom_vel_init){
			odom_vel_x_acc_filter.set_cutoff_frequency(6.0f);
			odom_vel_y_acc_filter.set_cutoff_frequency(6.0f);
			odom_vel_init=true;
		}
		odom_vel_x_acc+=get_accel_ef().x*100.0*_dt;
		odom_vel_y_acc+=get_accel_ef().y*100.0*_dt;
		if(get_optical_location){
			update_odom_time=HAL_GetTick();
			odom_vel_x_raw=ned_current_vel.x;
			odom_vel_y_raw=ned_current_vel.y;
			odom_pos_x=ned_current_pos.x;
			odom_pos_y=ned_current_pos.y;
			get_optical_location=false;
			update_pos=true;
//		usb_printf("pos:%f|%f\n",odom_pos_x,odom_pos_y);
		}else{
			odom_vel_x_raw+=get_accel_ef().x*100.0*_dt;
			odom_vel_y_raw+=get_accel_ef().y*100.0*_dt;
			if(HAL_GetTick()-update_odom_time>1000&&robot_state==STATE_STOP){//未起飞且定位源失效
		...
		odom_vel_x=odom_vel_x_acc_filter.apply(odom_vel_x_acc, _dt);
		odom_vel_y=odom_vel_y_acc_filter.apply(odom_vel_y_acc, _dt);
		get_odom_location=true;
	}else{
		get_odom_location=false;
	}
//	usb_printf("x:%f|y:%f, %f|%f\n",odom_pos_x,odom_pos_y, odom_vel_x,odom_vel_y);
#endif
}
...
void ekf_gnss_xy(void){
	...
	pos_control->set_accel_wind(ekf_wind->wind_x_filt, ekf_wind->wind_y_filt);
#endif
	if(get_odom_location){
		ekf_gnss->update(get_odom_location,odom_pos_x,odom_pos_y,odom_vel_x,odom_vel_y);
	}else{
...
static void update_throttle_hover(void)
{
    ...
    if (throttle > param->t_hover_update_min.value && throttle < param->t_hover_update_max.value && abs(climb_rate) < 60.0f && abs(roll_log_deg) < 5.0f && abs(pitch_log_deg) < 5.0f) {
        // Can we set the time constant automatically
        motors->update_throttle_hover(0.01f);
        param->rate_pid_integrator.value.x = constrain_float(param->rate_pid_integrator.value.x + 0.001*(attitude->get_rate_roll_pid().get_integrator()-param->rate_pid_integrator.value.x), -0.2f, 0.2f);
        param->rate_pid_integrator.value.y = constrain_float(param->rate_pid_integrator.value.y + 0.001*(attitude->get_rate_pitch_pid().get_integrator()-param->rate_pid_integrator.value.y), -0.2f, 0.2f);
        param->rate_pid_integrator.value.z = constrain_float(param->rate_pid_integrator.value.z + 0.001*(attitude->get_rate_yaw_pid().get_integrator()-param->rate_pid_integrator.value.z), -0.2f, 0.2f);
    }
}
...
void unlock_motors(void){
	...
	write_gpio2(true);
	Buzzer_set_ring_type(BUZZER_LOCK);
...
static uint32_t lock_detector_tick = 0;
static uint8_t lock_detector_count = 0;
		...
		if(rf_alt_raw>param->landing_lock_alt.value&&rf_alt_raw<30.0f&&get_vel_z()<0){
			if(rf_alt_raw!=rf_alt_raw_last){
				if(lock_detector_count==0){
					...
					if(HAL_GetTick()-lock_detector_tick<200){
						lock_detector_count++;
					}else{
						lock_detector_count=0;
					}
				}
				if(lock_detector_count>3){
					lock_detector_count=3;
					use_rf_alt_land=true;
				}
				rf_alt_raw_last=rf_alt_raw;
...
static uint8_t ch8_state=0,ch8_state_l=0,ch8_state_h=0;
void lock_motors_check(void){
	...
	if(ch8>0&&ch8<0.1){
		...
		ch8_state=0;
		disarm_counter++;
		if(disarm_counter>30){
			disarm_counter=30;
			ch8_state_l=0;
		}else if(disarm_counter>10){
			ch8_state_l=1;
		}else{
			ch8_state_l=0;
		}
...
void Logger_Data_Callback(void){
	...
	sd_log_write("%8.3f %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f ",//LOG_EULER
			log_pitch_deg(), log_roll_deg(), log_yaw_deg(), ahrs_pitch_deg(), ahrs_roll_deg(), ahrs_yaw_deg(), gps_position->heading, gps_position->lat_noise, gps_position->lon_noise, gps_position->alt_noise);
	osDelay(2);
...
void comm_send_callback(void){
	if(offboard_connected){
		...
		scaled_imu.xmag=(int16_t)(get_mag_filt().x*1000);
		scaled_imu.ymag=(int16_t)(get_mag_filt().y*1000);
		scaled_imu.zmag=(int16_t)(get_mag_filt().z*1000);
 ...
 *****************************************************************/
void debug(void){
//	usb_printf("rc:%d,gcs:%d\n",rc_channels_healthy(),get_gcs_connected());
//	usb_printf("l:%f\n",get_dis_wall());
...
//	usb_printf("uas:%ld|%d\n",param->real_name_reg.value, param->fs_uid_type.value);
...
//	usb_printf("wind_x_filt:%f|wind_y_filt:%f\n",ekf_wind->wind_x_filt,ekf_wind->wind_y_filt);
